...
use crate::{
    ...
    BoxShadow, Capslock, Context, Corners, CursorHideMode, CursorStyle, CustomShaderId,
    CustomShaderInstance, Decorations, DevicePixels, DispatchActionListener, DispatchNodeId,
    DispatchTree, DisplayId, Edges, Effect, Entity, EntityId, EventEmitter, FileDropEvent, FontId,
    Global, GlobalElementId, GlowParams, GlyphId, GpuSpecs, Hsla, InputHandler, IsZero, KeyBinding,
    ...
    MouseUpEvent, NativeDragIcon, NativeDragMode, NativeDragResult, Path, Pixels, PlatformAtlas,
    ...
    pub(crate) fn new(
        ...
        platform_window.on_request_frame(Box::new({
            ...
            move |request_frame_options| {
                ...
                // The updates below still fail ("RefCell already borrowed") if the
                // platform delivers the frame message re-entrantly through a path
                // the `draw_in_progress()` guard above does not cover — i.e.
                // something inside an update pumped the message queue outside of a
                // draw. The failure stack names that something, so capture it
                // (backtrace only materializes on error; if anyhow didn't capture
                // one at construction, the logger force-captures at the log site —
                // same stack here).
...
impl Window {
    ...
    pub fn paint_quad(&mut self, quad: PaintQuad) {
        ...
        // Quads carrying a per-element effect are exempt from the split below:
        // the effect is parameterized over the quad's own geometry (effect_type
        // 1 paints an outer glow around `bounds`), so four border strips would
        // paint four glows where the caller asked for one.
        if quad.effect_type != 0 {
            self.next_frame.scene.insert_primitive(quad);
            return;
        }
    ...
    ///
    /// Always stores the mapping so that external MCP tools can query element info
    /// even when the built-in inspector picker is not active.
    #[cfg(any(feature = "inspector", debug_assertions))]
