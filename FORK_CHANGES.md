...
| `16dc4c8b` | wgpu `render_to_image`: derive `premultiplied_alpha` from the surface alpha mode like `draw()` — the hardcoded 0 fed straight-alpha shader output into premultiplied-blending pipelines and bleached the whole offscreen frame toward white on PreMultiplied surfaces (inspector screenshots looked washed out while the live window was correct) |
