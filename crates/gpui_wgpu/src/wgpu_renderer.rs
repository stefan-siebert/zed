...
use gpui::{
    AtlasTextureId, Background, Bounds, CustomShaderId, DevicePixels, GpuSpecs, Path, Point,
...
impl WgpuRenderer {
            ...
            for batch in scene.batches() {
                match batch {
                    ...
                    PrimitiveBatch::Surfaces(range) => {
                        // Linux carries zero-copy DMABuf video frames here
                        // (elane-media). Upstream leaves this unimplemented on
                        // wgpu because only macOS/Metal had surface payloads.
                        #[cfg(all(target_os = "linux", not(target_family = "wasm")))]
                        {
                    ...
                    PrimitiveBatch::CustomShaders { shader_id, range } => self.draw_custom_shaders(
                        &instance_bindings.custom_shaders,
                        shader_id,
                        instance_range(range),
                        &mut pass,
                    ),
    ...
    fn write_instances(
        ...
        Ok(InstanceBindings {
            ...
            custom_shaders: self.write_instance_binding(
                "custom_shaders_bind_group",
                instance_offset,
...
"#,
    ...
    fn draw_custom_shaders(
        ...
        let Some(pipeline) = self.custom_shader_pipelines.get(&shader_id) else {
            return; // Unknown shader — skip silently
        };
        self.draw_instances(instances, pipeline, range, pass);
    }
    ...
    pub fn render_to_image(&mut self, scene: &Scene) -> anyhow::Result<image::RgbaImage> {
        ...
        // Render scene to offscreen texture (same structure as draw()).
        {
            let mut instance_offset = 0;
            let instance_bindings = self
                .write_instances(scene, &mut instance_offset)
                .context("scene too large for render_to_image")?;
...

            {
                ...
                for batch in scene.batches() {
                    match batch {
                        ...
                        // Video surfaces are live frames, not part of a still
                        // capture; backdrop blur is unimplemented on wgpu.
