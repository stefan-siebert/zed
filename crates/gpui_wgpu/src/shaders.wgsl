...
struct SurfaceParams {
    bounds: Bounds,
    content_mask: Bounds,
    // Column-major YCbCr -> RGB conversion for the frame's colorimetry
    // (applied to (Y, Cb, Cr, 1)); supplied per surface by the producer.
...
fn fs_surface(input: SurfaceVarying) -> @location(0) vec4<f32> {
    ...
    return surface_locals.ycbcr_to_rgb * y_cb_cr;
}
